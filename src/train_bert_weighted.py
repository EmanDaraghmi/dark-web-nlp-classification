...
def main():
    ...
    if text_col is None or label_col is None:
        raise ValueError(
            ...
            "Please inspect the dataset columns manually."
        )
    ...
    work["label"].value_counts().to_csv(TABLES_DIR / "weighted_bert_class_distribution.csv")
    ...
    training_args = TrainingArguments(
        output_dir="results/weighted_bert_model",
        eval_strategy="epoch",
        save_strategy="epoch",
        learning_rate=2e-5,
    ...
    print("Training Weighted BERT...")
    trainer.train()

    print("Evaluating Weighted BERT...")
    results = trainer.evaluate()
    pd.DataFrame([results]).to_csv(TABLES_DIR / "weighted_bert_results.csv", index=False)
    ...
    pd.DataFrame(report).transpose().to_csv(TABLES_DIR / "weighted_bert_classification_report.csv")
    ...
    plt.ylabel("Actual")
    plt.title("Weighted BERT Confusion Matrix")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "weighted_bert_confusion_matrix.png", dpi=300)
    plt.close()

    print("Done.")
    print("Weighted BERT results saved.")
    print(results)
